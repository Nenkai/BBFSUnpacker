...
namespace BBFSUnpacker.Hashing
{
    class CRC32
    {
        public static uint DoCRC(Span<byte> data, uint length, uint a3)
        {
            uint v3; // esi
            uint v4; // ebx
            uint v5; // edi
            ...
            if (v5 != 0)
            {
                do
                {
                    ...
                    v10 = CRC[v7] ^ v8 >> 8;
                    i += 3;
                    v11 = CRC[(byte)v10 ^ v9] ^ v10 >> 8;
                    v3 = CRC[(byte)v11 ^ data[i - 1]] ^ v11 >> 8;
                }
                while (i < v5);
                v4 = length;
            }
            for (; i < v4; ++i)
                v3 = CRC[(byte)v3 ^ data[i]] ^ v3 >> 8;
            //if (a3)
            //	*a3 = v3;
            return v3;
        }

        private static readonly uint[] CRC = new uint[]
        {
